        ...
        >
          <Menu>
            <Header size="300" style={{ padding: `0 ${config.space.S200}` }}>
              <Text size="L400">{t('pages:auth.login.hint')}</Text>
            </Header>
            <Box
        ...
        {loginState.status === AsyncStatus.Error && (
          <>
            {loginState.error.errcode === LoginError.ServerNotAllowed && (
              <FieldError message={t('pages:auth.login.login_with_custom_server_not_allowed')} />
            )}
            {loginState.error.errcode === LoginError.InvalidServer && (
              <FieldError message={t('pages:auth.login.failed_to_find_your_matrix_id_server')} />
            )}
          </>
        )}
      </Box>
